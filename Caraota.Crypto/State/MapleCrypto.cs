...
namespace Caraota.Crypto.State
{
    public class MapleCrypto : IMapleEncryptor, IMapleDecryptor
    {
        ...
        private static void Shuffle(byte inputByte, Span<byte> start)
        {
            byte s0 = start[0];
            byte s1 = start[1];
            byte s2 = start[2];
            ...
            byte b = _shiftTable[s1];
            b -= inputByte;
            s0 += b;

            byte shiftInput = _shiftTable[inputByte];
            s1 -= (byte)(s2 ^ shiftInput);
            ...
            uint c = (uint)(s0 | (s1 << 8) | (s2 << 16) | (s3 << 24));
